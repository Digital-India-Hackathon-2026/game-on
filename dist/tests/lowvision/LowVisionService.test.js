...
const LowVisionService_1 = require("../../lowvision/LowVisionService");
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
...
// Tests
// ---------------------------------------------------------------------------
const service = new LowVisionService_1.LowVisionService();
// ---- 1. Display Config ----
...
test("alt-text with missing input throws error", async () => {
    try {
        ...
        assert(err instanceof LowVisionService_1.LowVisionInputError, "should be LowVisionInputError");
        assertEqual(err.statusCode, 400, "should be 400");
    }
});
...
test("update-preferences validates boolean fields", async () => {
    try {
        await service.updatePreferences("test-user-bool", { boldText: "yes" });
        assert(false, "should have thrown");
    }
    catch (err) {
        assert(err instanceof LowVisionService_1.LowVisionInputError, "should be LowVisionInputError");
...
test("rate limiting works for preferences", async () => {
    ...
    const rateLimited = results.filter((r) => r instanceof LowVisionService_1.LowVisionRateLimitError);
