...
exports.AstigmatismService = exports.AstigmatismFetchError = exports.AstigmatismInputError = void 0;
...
class AstigmatismInputError extends Error {
    code = "astigmatism_input_error";
    statusCode = 400;
    constructor(message) {
        super(message);
...
exports.AstigmatismInputError = AstigmatismInputError;
class AstigmatismFetchError extends Error {
    code = "astigmatism_fetch_error";
    statusCode = 502;
    constructor(message) {
        super(message);
...
exports.AstigmatismFetchError = AstigmatismFetchError;
class AstigmatismService {
    health() {
    ...
    updateToggleState(userId, request) {
    ...
    injectCorrection(html, finalUrl, options = {}) {
    ...
    buildInjectionSnippet(severity, enabled) {
        const allCss = Object.entries(SEVERITY_VALUES)
            .map(([key, values]) => this.buildSeverityCss(key, values))
            .join("\n");
...
(function(){
  ...
  apply({enabled:${enabled ? "true" : "false"},severity:${JSON.stringify(severity)}});
    ...
    buildSeverityCss(severity, values) {
...
html.saralo-astigmatism-${severity} a,html.saralo-astigmatism-${severity} button,html.saralo-astigmatism-${severity} [role="button"],html.saralo-astigmatism-${severity} [role="link"]{font-size:calc(1em*${Math.max(1.12, values.bodyFontScale)})!important;text-decoration-thickness:max(2px,.08em)!important}`;
    ...
    async fetchHtml(url) {
        ...
        try {
            ...
            if (error instanceof AstigmatismFetchError)
                throw error;
    ...
    createPreviewPage(original, corrected, finalUrl, severity) {
        return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  ...
  <main class="preview">
    ...
    validateUrl(value) {
        ...
        try {
            ...
            if (error instanceof AstigmatismInputError)
                throw error;
...
exports.AstigmatismService = AstigmatismService;
