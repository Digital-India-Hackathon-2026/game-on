...
exports.ADHDService = exports.ADHDRateLimitError = exports.ADHDInputError = void 0;
const AIErrorHandler_1 = require("../ai/AIErrorHandler");
// ---------------------------------------------------------------------------
// Constants
// ---------------------------------------------------------------------------
...
const readingProgressStore = new Map();
...
function validateAndSanitizeInput(text, label) {
...
// Custom Errors
// ---------------------------------------------------------------------------
class ADHDInputError extends Error {
    code = "adhd_input_error";
    statusCode;
    constructor(message) {
        super(message);
...
exports.ADHDInputError = ADHDInputError;
class ADHDRateLimitError extends Error {
    code = "adhd_rate_limit";
    statusCode;
    constructor() {
...
exports.ADHDRateLimitError = ADHDRateLimitError;
// ---------------------------------------------------------------------------
// Rate limiter
// ---------------------------------------------------------------------------
...
async function callLLMWithFallback(systemPrompt, userText) {
    let lastError;
    for (let attempt = 0; attempt <= LLM_MAX_RETRIES; attempt++) {
        try {
            ...
            if (err instanceof AIErrorHandler_1.AIEngineError && err.code === "provider_failure") {
                return null;
            }
...
async function callLLMOnce(systemPrompt, userText) {
    try {
        ...
        const response = await Promise.race([
            ...
            new Promise((_, reject) => setTimeout(() => reject(new AIErrorHandler_1.AIEngineError("timeout", "LLM timed out.", 503)), LLM_TIMEOUT_MS)),
        ]);
        ...
        if (err instanceof AIErrorHandler_1.AIEngineError && err.code === "timeout")
            throw err;
        return await mockLLM(systemPrompt, userText);
    }
}
async function mockLLM(systemPrompt, userText) {
...
class ADHDService {
    ...
    async generateSummary(req) {
        ...
        const result = await callLLMWithFallback("You are a reading assistant for users with ADHD. Provide a short TL;DR summary (1-2 sentences) and 3-5 key points in JSON format: { \"tldr\": string, \"keyPoints\": string[] }. Be concise and direct.", cleanText);
        let response;
        if (result) {
            try {
    ...
    async chunkText(req) {
        ...
        const result = await callLLMWithFallback("You are a reading assistant for users with ADHD. Break the following text into short, digestible paragraphs (chunks) without changing the meaning. Return JSON: { \"chunks\": string[] }. Each chunk should be 1-3 sentences max. Make it easy to read.", cleanText);
        let response;
        if (result) {
            try {
                const parsed = JSON.parse(result);
    ...
    async createBookmark(userId, req) {
        checkRateLimit(userId);
    ...
    // ---- 7. Reading Progress ----
    async getReadingProgress(userId, url) {
        checkRateLimit(userId);
        if (!url)
            throw new ADHDInputError("url is required.");
    ...
    async saveReadingProgress(userId, req) {
        checkRateLimit(userId);
        if (!req.url || typeof req.url !== "string") {
            throw new ADHDInputError("url is required.");
        }
...
exports.ADHDService = ADHDService;
