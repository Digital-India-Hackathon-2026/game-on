...
import type {
  LowVisionDisplayConfig,
  ContrastPreset,
  LargerClickTargetConfig,
  AltTextRequest,
  AltTextResponse,
  LowVisionPreferences,
  LowVisionPreferencesUpdate,
  ReadAloudTextRequest,
  ReadAloudTextResponse,
  LowVisionHealthResponse,
} from "./LowVisionTypes";
...
const CONTRAST_PRESETS: ContrastPreset[] = [
  {
    id: "normal",
    label: "Normal",
    foreground: "#1a1a1a",
...
const DISPLAY_CONFIG: LowVisionDisplayConfig = {
...
const altTextCache = new Map<string, CacheEntry<AltTextResponse>>();
const preferencesStore = new Map<string, LowVisionPreferences>();
...
function validateAndSanitizeText(text: string, label: string): string {
...
function validateNumber(value: unknown, label: string, min: number, max: number): number {
...
function getCacheKeyForAltText(imageUrl?: string, imageContext?: string): string {
...
export class LowVisionInputError extends Error {
  readonly code = "lowvision_input_error";
...
export class LowVisionRateLimitError extends Error {
  readonly code = "lowvision_rate_limit";
  readonly statusCode: number;
  constructor() {
...
async function generateAltTextWithLLM(
...
async function callAltTextLLMOnce(
  imageUrl: string | undefined,
  imageContext: string | undefined,
): Promise<string | null> {
  try {
    ...
    if (err instanceof Error && err.message === "LLM timed out") throw err;
    return await mockAltTextLLM(imageUrl, imageContext);
  }
}

async function mockAltTextLLM(
  imageUrl: string | undefined,
  imageContext: string | undefined,
): Promise<string | null> {
...
const NAV_SECTION_MARKERS: string[] = [
  "navigation",
  "nav",
  "menu",
  "sidebar",
...
function isBoilerplateLine(line: string): boolean {
  ...
  if (lower.length < 15) return false; // Keep short lines, they might be headings
  if (NAV_SECTION_MARKERS.some((marker) => lower.includes(marker))) return true;
  return BOILERPLATE_PATTERNS.some((pattern) => pattern.test(line));
}

function cleanPageTextForReadAloud(rawText: string): {
  cleaned: string;
...
function createDefaultPreferences(userId: string): LowVisionPreferences {
  return {
    userId,
...
export class LowVisionService {
  // ---- 1. Display Config ----
  getDisplayConfig(): LowVisionDisplayConfig {
    return {
      ...DISPLAY_CONFIG,
  ...
  // ---- 2. Alt Text ----
  async generateAltText(req: AltTextRequest): Promise<AltTextResponse> {
  ...
  // ---- 3. Preferences (GET) ----
  async getPreferences(userId: string): Promise<LowVisionPreferences> {
  ...
  async updatePreferences(
    userId: string,
    update: LowVisionPreferencesUpdate,
  ): Promise<LowVisionPreferences> {
    if (!userId || typeof userId !== "string") {
      throw new LowVisionInputError("userId is required.");
    }
    checkRateLimit(userId);
    ...
    const updated: LowVisionPreferences = {
      ...current,
      userId,
      updatedAt: isoNow(),
    };
  ...
  // ---- 5. Read Aloud Text ----
  async cleanTextForReadAloud(req: ReadAloudTextRequest): Promise<ReadAloudTextResponse> {
  ...
  // ---- 6. Health ----
  health(): LowVisionHealthResponse {
    return {
      status: "ok",
      mode: "lowvision",
      timestamp: isoNow(),
